...
class SSWBASTFisik extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'ssw:bast_fisik';
     ...
     * @var string
     */
    protected $description = 'Untuk mengambil data BAST Fisik dari SSW';
    ...
    {
        if(filter_var(config('myconfig.scheduler.ssw_fisik'), FILTER_VALIDATE_BOOLEAN) == true)
        {
            DB::beginTransaction();
            try
            {
                // $getLastIdSSW = Permohonan::where('is_bast_fisik', true)->orderBy('id_ssw', 'desc')->first();
                ...
                $permohonanSSW = DB::table('permohonan_ssw')
                ->where('status', 2)->where('is_penarikan_psu', false)->get();
                ...
                foreach($permohonanSSW as $key => $value)
                {
                    ...
                    if(!empty($value->id_induk)) 
                    {
                        ...
                        if(!empty($checkParent) && $checkParent->status == 4)
                        {
                            $getInduk = Permohonan::where('id_ssw', $value->id_induk)->first();

                            if(!empty($getInduk))
                            {
                                $countPermohonan += Permohonan::where('id_induk_fisik_awal', $getInduk->id_induk_fisik_awal)->count();
                            }
                            else
                            {
                                $getInduk = null;
                    ...
                    $permohonan = Permohonan::where('id_ssw', $value->id)->first();

                    if(!empty($permohonan))
                    {
                        $permohonan->id_induk_fisik = (!empty($getInduk)) ? $getInduk->id : null;
                        $permohonan->permohonan_fisik_ke = (!empty($getInduk)) ? $countPermohonan : 1;
                    ...
                    else
                    {
                        ...
                        {
                            $getBerkas = Berkas::withoutGlobalScope('isActive')->where('id_ssw', $value2->id_berkas)->where('is_bast_fisik', true)->first();
                    ...
                    $permohonan->id_induk_fisik_awal = (!empty($getInduk)) ? $getInduk->id_induk_fisik_awal : $permohonan->id;
                ...
                DB::commit();
                $this->info('Data BAST Fisik Berhasil Di Ambil Dari SSW');
            }
            catch (\Exception $ex)
            {
                DB::rollBack();
        ...
        {
            $this->info('Scheduler SSWBASTFisik tidak aktif');
        }
    }
}
